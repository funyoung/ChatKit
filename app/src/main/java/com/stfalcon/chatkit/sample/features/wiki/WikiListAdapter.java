...
/**
 * Adapter for {@link WikiList}
 */
@SuppressWarnings("WeakerAccess")
public class WikiListAdapter
        extends RecyclerView.Adapter<WikiListAdapter.BaseDialogViewHolder> {

    protected List<DatasetItem> items = new ArrayList<>();
...
//    private DialogListStyle dialogStyle;
     ...
     */
    public WikiListAdapter(ImageLoader imageLoader) {
     ...
     * @param imageLoader  image loading method
     */
    public WikiListAdapter(@LayoutRes int itemLayoutId, ImageLoader imageLoader) {
     ...
     * @param imageLoader  image loading method
     */
    public WikiListAdapter(@LayoutRes int itemLayoutId, Class<? extends BaseDialogViewHolder> holderClass,
...
//            }
    ...
    public void deleteById(String id) {
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).getOid().equals(id)) {
                items.remove(i);
                notifyItemRemoved(i);
            }
        }
     ...
     */
    public void addItems(List<DatasetItem> newItems) {
        if (newItems != null) {
            if (items == null) {
                items = new ArrayList<>();
            }
    ...
    /**
     * Add wikiItem to the end of dialogs list
     *
     * @param wikiItem wikiItem item
     */
    public void addItem(DatasetItem wikiItem) {
        items.add(wikiItem);
        notifyItemInserted(items.size() - 1);
    }

    /**
     * Add wikiItem to dialogs list
     *
     * @param wikiItem   wikiItem item
     * @param position position in dialogs list
     */
    public void addItem(int position, DatasetItem wikiItem) {
        items.add(position, wikiItem);
    ...
    public void moveItem(int fromPosition, int toPosition) {
        DatasetItem wikiItem = items.remove(fromPosition);
        items.add(toPosition, wikiItem);
        notifyItemMoved(fromPosition, toPosition);
    }

    /**
     * Update wikiItem by position in dialogs list
     *
     * @param position position in dialogs list
     * @param item     new wikiItem item
     */
    public void updateItem(int position, DatasetItem item) {
        if (items == null) {
            items = new ArrayList<>();
        }
    ...
    /**
     * Update wikiItem by wikiItem id
     *
     * @param item new wikiItem item
     */
    public void updateItemById(DatasetItem item) {
        if (items == null) {
            items = new ArrayList<>();
        }
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).getOid().equals(item.getOid())) {
                items.set(i, item);
                notifyItemChanged(i);
                break;
            }
    ...
    /**
     * Upsert wikiItem in dialogs list or add it to then end of dialogs list
     *
     * @param item wikiItem item
     */
    public void upsertItem(DatasetItem item) {
     ...
     */
    @Nullable
    public DatasetItem getItemById(String id) {
        if (items == null) {
            items = new ArrayList<>();
        }
        for (DatasetItem item : items) {
            if (item.getOid() == null && id == null) {
                return item;
            } else if (item.getOid() != null && item.getOid().equals(id)) {
                return item;
            }
        }
        return null;
...
//    /**
//     * Update last message in wikiItem and swap item to top of list.
//     *
//     * @param dialogId Dialog ID
//     * @param message  New message
//     * @return false if wikiItem doesn't exist.
//     */
//    @SuppressWarnings("unchecked")
//    public boolean updateDialogWithMessage(String dialogId, IMessage message) {
//        boolean dialogExist = false;
//        for (int i = 0; i < items.size(); i++) {
//            if (items.get(i).getOid().equals(dialogId)) {
...
//    }
    ...
    /**
     * Sort wikiItem by last message date
     */
    public void sortByLastMessageDate() {
        Collections.sort(items, (o1, o2) -> {
            if (new Date(o1.getCreateTime()).after(new Date(o2.getCreateTime()))) {
                return -1;
            } else if (new Date(o1.getCreateTime()).before(new Date(o2.getCreateTime()))) {
                return 1;
            } else return 0;
        });
        notifyDataSetChanged();
     ...
     */
    public void sort(Comparator<DatasetItem> comparator) {
    ...
    /**
     * Register a callback to be invoked when wikiItem view is clicked.
     *
...
//    }
    ...
    /**
     * @return the position of a wikiItem in the dialogs list.
     */
    public int getDialogPosition(DatasetItem wikiItem) {
        return this.items.indexOf(wikiItem);
    }
    ...
    public interface OnDialogClickListener {
        void onDialogClick(phos.fri.aiassistant.entity.DatasetItem wikiItem);
    }

    public interface OnDialogViewClickListener {
        void onDialogViewClick(View view, phos.fri.aiassistant.entity.DatasetItem wikiItem);
    }

    public interface OnDialogLongClickListener {
        void onDialogLongClick(phos.fri.aiassistant.entity.DatasetItem wikiItem);
    }

    public interface OnDialogViewLongClickListener {
        void onDialogViewLongClick(View view, phos.fri.aiassistant.entity.DatasetItem wikiItem);
    }
    ...
    public abstract static class BaseDialogViewHolder
            extends ViewHolder<DatasetItem> {
...
//        }
        ...
        @Override
        public void onBind(final phos.fri.aiassistant.entity.DatasetItem wikiItem) {
//            if (wikiItem.getUnreadCount() > 0) {
//                applyUnreadStyle();
//            } else {
//                applyDefaultStyle();
//            }
            ...
            //Set Name
            tvName.setText(wikiItem.getDatasetName());

            if (wikiItem.getDatasetDesc() != null) {
                tvDate.setText(wikiItem.getCreateTime());
            } else {
                tvDate.setText(null);
            }
            ...
            //Set Dialog avatar
//            if (imageLoader != null) {
//                imageLoader.loadImage(ivAvatar, wikiItem.getDialogPhoto(), null);
//            }
//
//            //Set Last message user avatar with check if there is last message
//            if (imageLoader != null && wikiItem.getLastMessage() != null) {
//                imageLoader.loadImage(ivLastMessageUser, wikiItem.getLastMessage().getUser().getAvatar(), null);
//            }
//            ivLastMessageUser.setVisibility(dialogStyle.isDialogMessageAvatarEnabled()
//                    && wikiItem.getUsers().size() > 1
//                    && wikiItem.getLastMessage() != null ? VISIBLE : GONE);

            //Set Last message text
            if (wikiItem.getDatasetDesc() != null) {
                tvLastMessage.setText(wikiItem.getDatasetDesc());
            } else {
                tvLastMessage.setText(null);
            }

//            //Set Unread message count bubble
//            tvBubble.setText(String.valueOf(wikiItem.getUnreadCount()));
////            tvBubble.setVisibility(dialogStyle.isDialogUnreadBubbleEnabled() &&
////                    wikiItem.getUnreadCount() > 0 ? VISIBLE : GONE);

            container.setOnClickListener(view -> {
                if (onDialogClickListener != null) {
                    onDialogClickListener.onDialogClick(wikiItem);
                }
                if (onDialogViewClickListener != null) {
                    onDialogViewClickListener.onDialogViewClick(view, wikiItem);
                }
            });
            ...
            container.setOnLongClickListener(view -> {
                if (onLongItemClickListener != null) {
                    onLongItemClickListener.onDialogLongClick(wikiItem);
                }
                if (onDialogViewLongClickListener != null) {
                    onDialogViewLongClickListener.onDialogViewLongClick(view, wikiItem);
                }
